...
## Update repeat-prompt fix

- версия обновлена до `1.0.6+7`;
- APK и Windows Setup получают уникальное имя для каждой версии;
- устранено скачивание старого файла из кэша по постоянному URL;
- `version.json` использует платформенные блоки Android/Windows;
